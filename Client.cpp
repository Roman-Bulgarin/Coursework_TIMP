...
int Client::Server(string str1, string str2)
{
    ...
    // @details объект, selfAddr, инициализируется с использованием семейства адресов AF_INET, порт устанавливается в 0, а IP-адрес устанавливается в 0.
    sockaddr_in * selfAddr = new (sockaddr_in);
    selfAddr->sin_family = AF_INET;
    selfAddr->sin_port = 0;
    selfAddr->sin_addr.s_addr = 0;
	// @details объект, remoteAddr, также инициализируется с использованием семейства адресов AF_INET, порт устанавливается с использованием функции htons, которая преобразует 64-битное беззнаковое целое число из  сетевого порядка байтов в порядок байтов хоста, а IP-адрес устанавливается с использованием функции inet_addr, которая преобразует строку IP-адреса в 32-битное беззнаковое целое число.
    ...
    cout << termcolor::bold << termcolor::yellow << "Логин: " << buffer << termcolor::reset << endl;
    ...
    buffer[rc] = '\0';
    cout << termcolor::bold << termcolor::cyan << "Соль: " << buffer << termcolor::reset << endl;
    ...
    cout << termcolor::bold << termcolor::yellow << "Отправка хэша: " << buffer << termcolor::reset << endl;
    ...
    cout << termcolor::bold << termcolor::cyan << "Ответ об аутентификации: " << buffer << termcolor::reset << endl;
    ...
    cout << termcolor::bold << termcolor::yellow<< "Число векторов: " << len << termcolor::reset << endl;
    ...
    for(int l = 0; l < len; l++) {
        ...
        cout << termcolor::bold << termcolor::yellow << "Размер вектора: " << size << termcolor::reset << endl;
        ...
        cout << termcolor::bold << termcolor::yellow << "Отправка самого вектора: " << array << termcolor::reset << endl;
		...
		cout << termcolor::bold << termcolor::cyan<< "Результат вычислений: " << summa << termcolor::reset << endl;
		
		ofstream outputFile(result);
		outputFile << static_cast<int64_t>(summa);
		outputFile.close();
    }
